...
if groups_checked == total_groups and available_groups < no_of_groups:
    ...
    for i in range(nodes_per_group):
        f2.write(final_result[i][0] + "\n")                     # Hostfile Category 1 containing nodes from different group
    
    '''
    # 2nd category hostfile can be generated using below code : It generates nodes from same group
    for i in range(nodes_per_group):
        f2.write(final_result[0][i] + "\n")
